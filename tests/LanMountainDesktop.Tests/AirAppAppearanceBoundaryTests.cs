...
using AirAppIsolationAppearanceSnapshot = LanMountainDesktop.AirAppIsolation.Contracts.AirAppAppearanceSnapshot;
using AirAppIsolationAppearanceSnapshotRequest = LanMountainDesktop.AirAppIsolation.Contracts.AirAppAppearanceSnapshotRequest;
using AirAppIsolationMaterialSurfaceSnapshot = LanMountainDesktop.AirAppIsolation.Contracts.AirAppMaterialSurfaceSnapshot;
using AirAppIsolationJsonContext = LanMountainDesktop.AirAppIsolation.Contracts.AirAppIsolationJsonContext;
using MaterialColorPalette = LanMountainDesktop.Models.MaterialColorPalette;
using AirAppSdkAppearanceSnapshot = LanMountainDesktop.AirAppSdk.AirAppAppearanceSnapshot;

namespace LanMountainDesktop.Tests;

public sealed class AirAppAppearanceBoundaryTests
{
    [Fact]
    public void AirAppLoader_PrefersMaterialColorService_WhenBothAppearanceSourcesAreAvailable()
    {
    ...
    [Fact]
    public void AirAppIsolationJsonContext_RoundTripsAppearancePayloads()
    {
        var request = new AirAppIsolationAppearanceSnapshotRequest("session-42");
        var snapshot = new AirAppIsolationAppearanceSnapshot(
            ThemeVariant: "Dark",
            ...
            MaterialSurfaces: new Dictionary<string, AirAppIsolationMaterialSurfaceSnapshot>(StringComparer.OrdinalIgnoreCase)
            {
                ["WindowBackground"] = new AirAppIsolationMaterialSurfaceSnapshot("#FF101820", "#33557799", 18, 0.92)
            },
            WallpaperSeedCandidates: ["#FF123456", "#FF214365"]);
        var notification = new AirAppIsolationAppearanceChangedNotification(snapshot);

        var requestJson = JsonSerializer.Serialize(request, AirAppIsolationJsonContext.Default.AirAppAppearanceSnapshotRequest);
        var requestRoundTrip = JsonSerializer.Deserialize(requestJson, AirAppIsolationJsonContext.Default.AirAppAppearanceSnapshotRequest);

        var snapshotJson = JsonSerializer.Serialize(snapshot, AirAppIsolationJsonContext.Default.AirAppAppearanceSnapshot);
        var snapshotRoundTrip = JsonSerializer.Deserialize(snapshotJson, AirAppIsolationJsonContext.Default.AirAppAppearanceSnapshot);

        var notificationJson = JsonSerializer.Serialize(notification, AirAppIsolationJsonContext.Default.AirAppAppearanceChangedNotification);
        var notificationRoundTrip = JsonSerializer.Deserialize(notificationJson, AirAppIsolationJsonContext.Default.AirAppAppearanceChangedNotification);
    ...
    private static AirAppSdkAppearanceSnapshot InvokeLoaderSnapshotBuilder(IServiceProvider provider)
    {
        var method = typeof(AirAppLoader).GetMethod(
        ...
        return Assert.IsType<AirAppSdkAppearanceSnapshot>(result);
    }

    private static void AssertAppearanceSnapshotEqual(AirAppIsolationAppearanceSnapshot expected, AirAppIsolationAppearanceSnapshot actual)
    {

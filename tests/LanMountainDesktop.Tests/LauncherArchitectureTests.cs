...
public sealed class LauncherArchitectureTests
{
    ...
    public void HostUpdateFlow_DoesNotDelegateApplyOrRollbackToLauncher()
    {
        var guardedFiles = new[]
        {
            Path.Combine(RepoRoot, "desktop", "LanMountainDesktop", "Services", "Update", "UpdateInstallGateway.cs"),
            Path.Combine(RepoRoot, "desktop", "LanMountainDesktop", "Services", "Update", "UpdateOrchestrator.cs")
        };
    ...
    private static string LauncherProjectRoot => Path.Combine(RepoRoot, "desktop", "LanMountainDesktop.Launcher");

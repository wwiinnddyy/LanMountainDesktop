...
`IAirApp` 是所有轻应用的入口接口，定义了轻应用的基本信息和生命周期方法。
...
public interface IAirApp
{
    ...
    /// <summary>
    /// 轻应用初始化
    /// 在轻应用加载后调用，用于注册组件、服务和事件
    /// </summary>
    /// <param name="context">轻应用上下文</param>
    /// <returns>异步任务</returns>
    Task InitializeAsync(IAirAppRuntimeContext context);
    
    /// <summary>
    /// 轻应用关闭
    /// 在轻应用卸载前调用，用于清理资源和保存状态
    /// </summary>
    /// <returns>异步任务</returns>
    Task ShutdownAsync();
}
...
// ❌ 不好的示例
"WeatherAirApp"              // 不是反向域名格式
"com.example.Weather AirApp" // 包含空格
...
**说明**: 轻应用的版本号，应遵循[语义化版本](https://semver.org/lang/zh-CN/)规范。
...
**说明**: 轻应用加载后立即调用，用于初始化轻应用、注册组件和服务。

**参数**:
- `context`: 轻应用上下文，提供对宿主服务的访问
...
private void RegisterServices(IAirAppRuntimeContext context)
{
    // 注册轻应用内部服务
...
public async Task InitializeAsync(IAirAppRuntimeContext context)
{
    ...
    catch (FileNotFoundException ex)
    {
        ...
        throw new AirAppInitializationException(
            "轻应用初始化失败：缺少必需文件",
            ex
        );
    }
...
{
    _logger.LogWarning($"AirApp {plugin.Name} shutdown timeout");
    // 强制终止
}
```
...
public class AirApp : IAirApp
{
    ...
    public string Name => "Minimal AirApp";
    public string Version => "1.0.0";
    
    public Task InitializeAsync(IAirAppRuntimeContext context)
    {
...
public class WeatherAirApp : IAirApp
{
    ...
    public async Task InitializeAsync(IAirAppRuntimeContext context)
    {
        try
        {
            ...
            _logger.LogInformation(
                "{AirAppName} v{Version} is initializing...",
                Name,
                Version
            );
            ...
            _logger.LogInformation(
                "{AirAppName} initialized successfully",
                Name
            );
        ...
        catch (Exception ex)
        {
            context.Logger.LogError(
                ex,
                "Failed to initialize {AirAppName}",
                Name
            );
            throw;
        }
    ...
    public async Task ShutdownAsync()
    {
        ...
        catch (Exception ex)
        {
            _logger?.LogError(
                ex,
                "Error during {AirAppName} shutdown",
                Name
            );
            // 不抛出异常
        }
    ...
    private void SubscribeToHostEvents(IAirAppRuntimeContext context)
    {
        var eventBus = context.Services.GetService<IEventBus>();
        if (eventBus != null)
        {
...
**A**: 轻应用会被标记为"加载失败"，不会被激活，但不影响其他轻应用。
...
- [IAirAppRuntimeContext 详解](02-IAirAppRuntimeContext.md) - 轻应用上下文
- [轻应用生命周期](../02-核心概念/01-轻应用生命周期.md) - 生命周期详解
- [创建第一个轻应用](../01-快速开始/02-创建第一个轻应用.md) - 实战教程

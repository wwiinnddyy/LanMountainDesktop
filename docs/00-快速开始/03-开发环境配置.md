...
本指南将帮助你配置阑山桌面的开发环境，以便进行轻应用开发、组件开发或贡献核心代码。
...
├── desktop/LanMountainDesktop             # 主程序
├── desktop/LanMountainDesktop.Launcher    # 启动器
├── airapp/LanMountainDesktop.AirAppSdk    # 轻应用 SDK
├── airapp/LanMountainDesktop.AirAppRuntime# Air APP 运行时
├── airapp/LanMountainDesktop.AirAppHost   # Air APP 宿主
├── core/LanMountainDesktop.Core           # 共享契约、IPC
└── tests/LanMountainDesktop.Tests         # 测试项目
```
...
| `LanMountainDesktop/plugins/` | 轻应用运行时 |
| `scripts/` | 构建和发布脚本 |
| `docs/` | 文档 |
...
```powershell
# 创建新轻应用
dotnet new lmd-airapp -n MyTestAirApp

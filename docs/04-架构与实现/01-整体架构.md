...
| 项目 | 职责 | 类型 |
|------|------|------|
| **LanMountainDesktop.AirAppSdk** | 轻应用 SDK（含轻应用隔离 AirAppIsolation、宿主接口 Host.Abstractions、设置 Settings.Core、外观 Appearance） | NuGet 包 |
| **LanMountainDesktop.Core** | 共享契约类型 + IPC 基础设施 + 轻应用打包（合并自 Shared.Contracts / Shared.IPC / AirAppPackaging） | NuGet 包 |
...
当前生产 AirAppHost 只按 `appId` 选择编译在程序中的 `world-clock`、`whiteboard`、`rss-reader` 视图，没有第三方 `airapp.json` 扫描、程序集加载或 SDK 生命周期装配。`.laapp` 扩展名当前由轻应用安装链路使用并要求包内存在 `airapp.json`；AirAppDevServer 以 `airapp.json` 为基础生成的同名扩展包不能作为生产 Air APP 安装包使用。

### 功能模块

| 模块 | 位置（合并后） | 说明 |
|------|------|------|
| **设置系统** | `LanMountainDesktop.AirAppSdk`（原 Settings.Core） | 设置持久化与 scope |
| **主题与外观** | `LanMountainDesktop.AirAppSdk`（原 Appearance） | 主题、圆角资源 |
| **桌面宿主生命周期** | `LanMountainDesktop` 主程序（原 DesktopHost） | DesktopBootstrap、启动协调、关闭协调 |
| **组件运行时** | 已删除（原 DesktopComponents.Runtime 空壳） | — |

### 未来扩展（计划中）

| 项目 | 职责 | 状态 |
|------|------|------|
| **轻应用隔离（AirAppIsolation.Contracts/Ipc）** | 轻应用隔离契约与 IPC | 已并入 `LanMountainDesktop.AirAppSdk` |

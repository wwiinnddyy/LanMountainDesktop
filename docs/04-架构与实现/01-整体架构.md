...
| 项目 | 职责 | 类型 |
|------|------|------|
| **LanMountainDesktop.PluginSdk** | 插件 SDK（含插件隔离 PluginIsolation、宿主接口 Host.Abstractions、设置 Settings.Core、外观 Appearance） | NuGet 包 |
| **LanMountainDesktop.Core** | 共享契约类型 + IPC 基础设施 + 插件打包（合并自 Shared.Contracts / Shared.IPC / PluginPackaging） | NuGet 包 |
...
| 模块 | 位置（合并后） | 说明 |
|------|------|------|
| **设置系统** | `LanMountainDesktop.PluginSdk`（原 Settings.Core） | 设置持久化与 scope |
| **主题与外观** | `LanMountainDesktop.PluginSdk`（原 Appearance） | 主题、圆角资源 |
...
| 项目 | 职责 | 状态 |
|------|------|------|
| **插件隔离（PluginIsolation.Contracts/Ipc）** | 插件隔离契约与 IPC | 已并入 `LanMountainDesktop.PluginSdk` |

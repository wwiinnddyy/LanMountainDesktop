...
写盘后回灌——一次收掉三族）。两条复用判定都写在这里：刚探过盘（窗口由调用方填，两家都填 400ms）就不必再 Stat，
以及磁盘写盘时间与缓存记的那次相同就复用；两处判定各自都有一种错法，且两种错法都<b>返回同一个快照</b>，
所以单看返回值量不到——行为钉 `SettingsSnapshotCacheTests` 6 格（含"换路径不许复用"和"取出的是副本、存进去的也是副本"）。
<b>锁与临界区留在两个服务里</b>：两边的失效粒度不同（一个跟着写盘事务，一个跟着 launcher 重载），
那是 #G1-T 里没被这条收掉的剩余部分。
...
组件的 `RefreshFromSettings` 读的是自己的设置项（时钟、课程表、可移动存储、白板等），**不是这条流程、没并进来**。
...
只许降不许升——当前 **59 组逐字相同的方法体（脚本同数）、192 个同名不同体的漂移族**，
外加两条下限："漂移普查认领量 ≥5400"（今天实测 5456）与"空实现 ≥28 处"。
...
**64 → 59 是 2026-09-23 一天里连收四族**（每一笔都单独写进了常量注释）：5 个组件各抄 7 行的
"只在黑夜档真的翻了才重画"进 `ComponentThemeMode.RefreshNightVisualIfChanged`（64 → 63）、
每日一词 1x1 与 2x2 那两份**逐字相同**的 13 行进 `DailyWordAutoRefresh.Apply`（63 → 62，
顺带把编辑器注册表里第三份默认档 `360` 指回这个常量）、头像占位字两份逐字 15 行进
`Services/Monogram.cs`（62 → 61）、路径存在性归一两对逐字 17 行进 `Shared/IO/ExistingPath.cs`
（61 → 59，跨 Core 与宿主、10 个调用点——两份算的都是"这条路径能不能信"，漂开的后果是
同一个安装被启动器与宿主判成两种结论）。同一天还有第五笔收口（7 个组件各 14 行的起停判据进
`ComponentRefreshLifetime.Reschedule`）**族数不动**，理由见下面那段"收益不在普查数上"。
...
这么一条转手调用，按行数算它仍是"一族两行的逐字相同"，族数一点没降（86）；
按语句数算才是真相：**74**。86→74 里 12 族是判据修正去掉的单语句转手，收口本身对族数无净影响，
它的真实收益是"钳到最少 1 格再重排"这段逻辑从 18 处各写一遍变成只有一处。
**"脚本 196 / C# 193、差 3 族没对过"这条已结清**（2026-09-23）：不是口径差，是两边各有一处判据 bug。
① 表达式体只认"签名行尾的 `=>`"，两种真实写法被整条丢掉——Allman 箭头体
（`private void UpdateLanguageCode()` 换行 `=> 表达式`）与插值字符串里的 `{message}` 被当成方法体开括号
（实测 `AirAppRuntimeLogger.cs:7` 的 `Info` 一路扫到类末尾）。② 脚本把"没有体的声明"（接口方法、abstract）
算成一种"体"，于是凭空多出 `nameof`、`VALUES` 这类假体族。两边都按同一规则改完后
**逐处对齐：同为 192 族、5449 处声明，站点清单（文件+行号）完全相同**——
这是这把尺子第一次做到"两个实现互相印证"，以后对数就比站点清单，别比族数。
组成变化分开记（全是改判据，一行代码都没收口）：掉出 `nameof`/`VALUES`/`SelectionOption`/`UpdateMonitoringLeaseState`
（假体挤掉的假阳性）；进来 `GetSessionsAsync`/`GetThemeBrush`/`SetValue`（接口默认实现真有多种写法，以前被吞没）。
教训：**只冻族数会被"判据瞎了"骗过去**——那次丢 67 处声明时族数看着像又收口了一档（193→189），
所以现在同时冻"认领量下限"，变异验证过：把 Allman 箭头体重新判丢，认领量 5449→5382，直接红。
**而"互相印证"有一条前提：两边的判据必须是各写一遍的**（2026-09-24 学到的）。C# 闸门与两份 python 探针
共用同一段"并签名续行"的逻辑，那段写的是"括号不等就把下一行并进来"，于是 `});` 这类**多闭合**的收尾行
被当成没闭合，连着吃掉紧跟其后的声明——实测全仓"本行无 `{` 且括号不等"的 7960 行里 3796 行是多闭合。
被吃掉的那条恰好是 Core 与启动器安装器里逐字相同的 27 行 `Retry`。上一笔我拿"两面族数与站点清单完全相同"
把"Retry 不在逐字面"判成"它只是漂移面的一个族"，**判据自己瞎着，互证一路绿**——共用逻辑的那一段互证无效，
只有已知正例喂它才算验证。改成"只在左括号多于右括号时并"之后：逐字面 31 → **33**（新可见 `Retry`、
`TryRemoveExistingPackage`、`ResolveAsStreamForReadMethod`；`DeleteFileWithRetry` 反而掉出，因为判据改正后
它的体是"单语句转手调多行 lambda"，正是"语句数 ≥2"要排除的形状），漂移面 182 → **193**（同笔、+11 族显形），
认领量下限那条断言同轮仍绿（所以不是判据放宽）。
**"没有体"其实有两种，混起来会让一整类缺陷隐身**（2026-09-23）：接口方法 / abstract 的**声明**没有实现，
不该算一种体；但 `void Foo() { }` 是**有实现、什么都不做**，必须算站点。第一版把两者都当"没体"跳过，
于是全仓 28 处空实现全体隐身——其中两个是组件契约：`RssReaderWidget.ApplyCellSize(double) { }`
（44 个组件都按格子换算字号，它整个空着：RSS 的字号与 WrapPanel 的 220×88 单元格在任何格子大小下都不变）
与 `ZhiJiaoHubWidget.SetRecommendationInfoService(…) { }`（宿主把推荐服务推进来，它收下就丢、从不存字段）。
两个都属"声明了契约却不执行"。其中 ZhiJiaoHub 那个已经修掉：宿主在 `DesktopComponentRuntimeRegistry.cs:206` 确实会把应用级服务推进来，组件却收下就丢、一直用自己 `new RecommendationDataService()`（宿主配的镜像/缓存目录对它无效），现在按 peers 的写法存字段。`RssReaderWidget.ApplyCellSize` 的缩放要怎么缩属视觉设计，登记为待办，不代做。判据自己也得防：
闸门里钉了"空实现 ≥28 处"的下限，把 `{ }`/`{}` 的规范化去掉就会红（实测只剩 11 处，一眼看出区分被合回去）。
移植中还踩到一个自己的错：给 for 循环加"吃完方法体就跳过去"时忘了给 `body.Count < 2` 那条分支推进下标，
测试宿主直接挂死（两分钟不动才发现）——按行推进的扫描器，每一条 continue 都得自己走那一步。
...
（一处没掉），而普查的族数/站点数**一动不动**（192 族 / 1371 处 / 当时逐字 64 组，现在的上限见上文）——

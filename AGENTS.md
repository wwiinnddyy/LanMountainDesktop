...
同日下午的存量：删掉 16 条纯遗留（合计 216 行，族②9 条 + 族③/④7 条）后
**A 级 28 / T 级 6，未解释各 5**。判为"实现了没入口"的登记在脚本 EXPLAINED 里，分待办
G1-AW（分类条手势）、G1-AY（隐私同意只写不读）、G1-AZ（启动进度链）、G1-BA（磁盘余量/
单场学习报告/常用时区表三条能力没入口）、G1-U（Core 公开面）；**没一条被顺手删**。
